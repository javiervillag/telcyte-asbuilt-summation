...
from app.rate_cards import CODE_PATTERN, UNIT_PATTERN, CodeKey, TotalKey, load_code_catalog, total_line_key
...
def _model_total_line_key(line: str) -> TotalKey | None:
    if not re.fullmatch(
        rf"\s*{CODE_PATTERN.pattern}\s*-\s*[0-9]+(?:\.[0-9]+)?(?:\s*{UNIT_PATTERN})?\s*",
        line,
        re.I,
    ):
        return None
