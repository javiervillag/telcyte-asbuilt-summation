...
- Confirmed 48Ct trunk-fiber sheets can treat the cable as a sequence: the Tie Point callout's T (tail) jacket number and the EOL callout's T number bound the cable, so their difference is the raw footage (storage, risers, and end slack are already inside the span). Jacket markers accept `D`/`T` plus pole-top `Top`/`Pole`/`P` prefixes, with or without a dash before the digits. When a splice or multiple tie point/EOL markers suggest multiple cables through the same route, the composite path-code callouts (`Comp-15`/`Comp-10` by default) are used instead. When both methods are available, their rounded order quantities must agree exactly or the Materials line is marked `VERIFY` and the run requires review. The sequence rule remains disabled for 144Ct and 288Ct until Nick confirms it applies to those cable types.
