...
def test_cable_footage_flags_default_safe() -> None:
    ...
    assert settings.cable_path_code == "Comp-15,Comp-10"
    assert settings.cable_path_code_list == ["Comp-15", "Comp-10"]
...
def test_cable_path_codes_are_trimmed() -> None:
    settings = Settings(CABLE_PATH_CODE=" Comp-15, , Comp-10 ")
    assert settings.cable_path_code_list == ["Comp-15", "Comp-10"]
